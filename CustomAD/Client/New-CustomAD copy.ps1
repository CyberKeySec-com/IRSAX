...
if (-not $scriptState.Step2_Completed) {
    try {
        function Set-NetworkConfiguration {
            ...
            $currentGateway = Get-NetRoute -InterfaceAlias $adapter.InterfaceAlias -DestinationPrefix "0.0.0.0/0" | Select-Object -ExpandProperty NextHop -ErrorAction SilentlyContinue
            ...
            $newDNSConfig = @("127.0.0.1", $SecondaryDNSServer | Where-Object { $_ -ne "127.0.0.1" -and $_ -ne $SecondaryDNSServer })
            if ($currentDNSConfig.ServerAddresses -ne $newDNSConfig) {
                ...
                Log-Message "DNS configuration set to Primary: 127.0.0.1, Secondary: $SecondaryDNSServer"
            } else {
                Log-Message "DNS configuration is already set to Primary: 127.0.0.1, Secondary: $SecondaryDNSServer"
            }
        }
        ...
        $config_secondary = @{
            # Parameters = @{
            #     OutputDirectory = ($config.Parameters.OutputDirectory).ToString()                
            #     InputDirectory  = ($config.Parameters.InputDirectory).ToString()
            #     LogFilePath     = ($config.Parameters.LogFilePath).ToString()
            # }
            Parameters = @{
...
while (-not (Get-ADDomainController)) {
